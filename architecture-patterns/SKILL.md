...
Master proven backend architecture patterns including Clean Architecture, Hexagonal Architecture, Domain-Driven Design, and deep module design to build maintainable, testable, and scalable systems.
...
- Evaluating whether a module is deep or shallow, and whether a seam is justified
- Setting up a domain glossary (`CONTEXT.md`) for a project

## Module Depth (Ousterhout / Pocock)

Before choosing an architecture pattern, understand the design goal that all patterns serve: **depth**. Every architectural decision should make modules deeper — more capability behind simpler interfaces.

### Core Vocabulary

- **Module** — anything with an interface and an implementation (function, class, package, slice). Scale-agnostic.
- **Interface** — everything a caller must know: types, invariants, error modes, ordering constraints, performance characteristics. Not just the type signature.
...
- **The interface is the test surface.** Callers and tests cross the same seam. If you need to reach past the interface to test, the module is the wrong shape.
- **One adapter = hypothetical seam. Two adapters = real seam.** Don't introduce a seam unless something actually varies across it.

### How This Relates to Architecture Patterns

Clean Architecture, Hexagonal, and DDD all create seams. The deep module lens helps you decide **which seams are justified**:
- A repository interface with a Postgres adapter AND an in-memory test adapter = real seam (two adapters).
- A `UserServiceInterface` with exactly one `UserServiceImpl` and no test fake = shallow indirection. Inline it until a second adapter is needed.

## Domain Glossary Convention: CONTEXT.md

Maintain a `CONTEXT.md` at the project root as a **glossary of domain terms** — not a spec, not a scratchpad, just the shared language.

### Why

- Agents and developers use consistent naming for variables, functions, and files
- Reduces verbosity (agents don't explain terms they already know)
...
**Order**:
A customer's request to purchase items. Created when checkout completes, not when items are added to cart.
_Avoid_: purchase, transaction (use only when referring to payment processing)

**Tenderer**:
A company submitting a response to an RFP. Each tenderer submits multiple deliverables.
_Avoid_: vendor, supplier, bidder

## Relationships

- A **Tenderer** submits many **Deliverables**
- A **Deliverable** is evaluated against one **Rubric**

## Flagged ambiguities

- [Record terms that were clarified during development]
```

### Rules

- Create `CONTEXT.md` lazily — only when the first term needs capturing
- Keep it devoid of implementation details
- When a new term emerges during development, add it immediately
- When a term conflicts with existing glossary, call it out and resolve
